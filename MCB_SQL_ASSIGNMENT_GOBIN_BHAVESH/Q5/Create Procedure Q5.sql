create or replace procedure THIRD_HIGHEST_ORDER_TOTAL_AMOUNT
  AS  
   c1 SYS_REFCURSOR;  
  BEGIN 
    open c1 for
    WITH selection_criteria AS (
      ...
      SELECT "Order Reference", "Order Date", "Supplier Name", "Order Total Amount", "Order Status","Invoice Reference"  from selection_criteria WHERE RNUM = 3 ;
