create or replace procedure bcm_migration AS
    ...
    RETURN VARCHAR2 IS 
       v_OUTPUT VARCHAR2(50);
        BEGIN
          v_OUTPUT := NVL(v_PHONENUMBER,''); -- set nulls to ''
          v_OUTPUT := REPLACE(REPLACE(v_OUTPUT, CHR(13), ''), CHR(10), ''); -- Removes carriage returns and new lines in case if any
          v_OUTPUT := REPLACE( v_OUTPUT, ' ', '' ); -- Removes spaces
          v_OUTPUT := REPLACE( v_OUTPUT, 'o', '0' ); -- change o to 0
          v_OUTPUT := REPLACE( v_OUTPUT, 'O', '0' ); -- change O to 0
          v_OUTPUT := REPLACE( v_OUTPUT, '.', '' ); -- Removes spaces
          RETURN v_OUTPUT;
        END;